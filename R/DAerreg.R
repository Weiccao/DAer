...
DAer_est <- function(dataset, impY, Kn = seq(0.1,0.9,0.1),
  ...
  m <- expectreg.ls(formula, dat0,
                    expectiles = Kn, quietly=TRUE)
  beta1 <- matrix(unlist(m$coefficients),length(Kn),
...
DAer <- function(dataset, imptau = seq(0.01,0.99,0.01), Kn = seq(0.1,0.9,0.1),
  ...
  for(h in 1:H){
    ...
    result <- DAer_est(dataset, imp.y, Kn = Kn, formula = formula)
