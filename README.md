# Overview
This document demonstrates how to run the DAer simulation study for censored expectile regression models.

It integrates data generation, estimation, summarization, and visualization into a complete pipeline.

## Note:

The parameter values here are only for demonstration purposes:

- K = 10 replications per setting

- scenarios_to_run <- paste0("sim", 1:4) (four scenarios)

In the official experiment, use:

- K = 1000

- scenarios_to_run <- paste0("sim", 1:8)
...
library(dirttee)
library(dplyr)
# Package ‘dirttee’ and 'expectreg' was removed from the CRAN repository.
# You can find it and download manually from:
# [https://cran.r-project.org/web/packages/dirttee/index.html]
# For 'expectreg':
# install_version("expectreg", version = "0.53", # Or latest archived version
# repos = "http://cran.us.r-project.org",
# type = "source")
```

We also need three supporting modules:

- datagen_module.R: defines data generation and censoring functions

- estimation_module.R: implements DAer, IPW, and Full estimation methods

- plot_module.R: provides plotting and visualization utilities
...
# Parameter Settings
The following block defines all key parameters for the simulation study.
```{r}
K <- 10                      # Number of replications per setting (demo value)
scenarios_to_run <- paste0("sim", 1:4)  # Scenarios to simulate
n_obs_list <- c(100, 200, 400)          # Sample sizes
censor_rate_list <- c(0.1, 0.2, 0.3)    # Censoring rates
target_Tau <- seq(0.1, 0.9, 0.1)        # Target expectile levels
H_val <- 20                             # Number of DAer imputation cycles
m_Tau_val <- 99                         # Number of imputation quantiles
```

# Function 1 : run_single_simulation()

This function runs one simulation replication for a given combination of sample size, scenario, and censoring rate.

It performs:

1. Data generation (generate_data)

2. Censoring (apply_censoring_auto or apply_censoring_fixed)

3. Expectile regression estimation (DAer, Full, and IPW if applicable)

4. Returns all estimation results and metadata.
...
# Function 2: summarize_scenario_results()
After multiple replications are completed, this function:

1. Loads all raw .Rdata results

2. Calculates average bias and MSE for β and Yτ

3. Computes average computation time

4. Exports results to Excel (openxlsx)
...
summarize_scenario_results <- function(scenario, n_obs_list, censor_rate_list, target_Tau) {
  ...
  # --- Save to Excel ---
  wb <- createWorkbook()
  addWorksheet(wb, "BIAS_beta")
  writeData(wb, "BIAS_beta", final_df %>% 
              select(n_obs, censor_rate, method, tau, starts_with("BIAS_beta.")))
  addWorksheet(wb, "MSE_beta")
  writeData(wb, "MSE_beta", final_df %>% 
              select(n_obs, censor_rate, method, tau, starts_with("MSE_beta.")))
  addWorksheet(wb, "BIAS_y")
  writeData(wb, "BIAS_y", final_df %>% 
              select(n_obs, censor_rate, method, tau, BIAS_y))
  addWorksheet(wb, "MSE_y")
  writeData(wb, "MSE_y", final_df %>% 
              select(n_obs, censor_rate, method, tau, MSE_y))
...
for (scenario in scenarios_to_run) {
  for (n_obs in n_obs_list) {
    for (cr in censor_rate_list) {
      ...
      if (is.null(example_preview)) {
        example_preview <- replication_results_list[[1]]$DAer$finalbeta
      }
    }
  }
}

cat("All simulations are complete. Raw data saved in 'results/raw_data'.\n")

# show the demonstration results
print(head(example_preview))
```

# Summarize and Plot Results
```{r summarize-and-plot, fig.width=7, fig.height=5}
